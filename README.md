...
- **Caps negotiation** — every element declares a constraint over its link caps (dtype, bands, CRS, resolution range, chunk size), per chunk kind (raster, point cloud or vector). A solver adapted from [glass2glass](https://github.com/Glass2GlassHQ/glass2glass) fixates the graph or fails naming the incompatible link.
