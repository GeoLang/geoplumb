...
- `VecSrc` holds the whole collection resident and re-simplifies every feature per tile read. A whole-window vector pull returns clipped fragments, features spanning tile seams stay split until a dissolve element exists. A segment passing exactly through a cell corner is a float tie and may burn one corner-adjacent cell differently between chunked and whole runs.
