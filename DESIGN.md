...
Caps are kinded: a `CapsPattern` is a raster pattern (dtype, bands, crs, resolution range, chunk px), a point cloud pattern or a vector pattern (both crs, resolution, chunk px), and cross-kind intersection is empty, so a link always negotiates to one kind and a point source wired straight into a raster consumer fails with the usual empty-link error. A `Derived` whose output pattern has a different kind than its input is a cross-kind transform (`IdwGrid`: points in, raster out; `Rasterize`: features in, raster out). Its passthrough mask projects only the fields every kind shares, crs, resolution and chunk size, which is what lets a crs demand downstream of a gridder narrow the point link, and lets the raster-side reproject auto-plug fire downstream of it. New kinds (tensor) extend the pattern enum and the common-field projection, the solver stays kind-blind.
